...
<div id="<?php echo esc_html( 'sptp-' . $generator_id ); ?>"  class="sp-team sptp-carousel sptp-section <?php echo 'sptp-' . esc_html( $page_link_type ); ?>">
	...
	if ( ! empty( $filter_members ) ) :
		...
		<?php
		$show_pagination = isset( $settings['carousel_pagination'] ) ? $settings['carousel_pagination'] : true;
		if ( $show_pagination && ( count( $filter_members ) > $desktop ) ) :
			?>
			<div class="sptp-pagination swiper-pagination"></div>
		<?php endif; ?>
		<?php
		$show_navigation = isset( $settings['carousel_navigation'] ) ? $settings['carousel_navigation'] : true;
