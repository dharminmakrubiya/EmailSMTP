...
class Helper {
	 ...
	 * @return void
	 */
	public static function member_description( $member_info, $show_member_bio, $member ) {
		...
		if ( $show_member_bio ) {
			...
			if ( false !== $cached_data ) {
				...
				self::sptp_set_transient( $cache_key, $description );
			}
	...
	public static function sptp_html_show( $generator_id, $layout, $settings, $main_section_title, $is_not_preview = true ) {
		...
		if ( ! $css_load_in_head && $is_not_preview ) {
			$final_css = '';
			// Include dynamic CSS to the section.
			include 'partials/dynamic-style.php';
			echo apply_filters( 'sp_team_free_dynamic_css', '<style>' . $final_css . '</style>' );
		}

		if ( ! empty( $layout_preset ) ) {
			switch ( $layout_preset ) {
				case 'carousel':
					if ( $sptp_swiper_css && ! $css_load_in_head && $is_not_preview ) {
