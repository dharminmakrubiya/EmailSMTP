...
class Admin {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->sptp_team_name            = ( ! empty( $this->sptp_options['rename_team'] ) ) ? $this->sptp_options['rename_team'] : __( 'Team', 'team-free' );
		...
		Generator::output_metaboxes( '_sptp_generator_output' );
		// Database updater.
		new DB_Updater();
		new SPTP_Preview();
	}
	...
	public function enqueue_styles() {
		...
		if ( 'sptp_generator' === $current_post_type ) {
			wp_enqueue_style( 'team-free-swiper', SPT_PLUGIN_ROOT . 'src/Frontend/css/swiper.min.css', array(), SPT_PLUGIN_VERSION, 'all' );
			wp_enqueue_style( SPT_PLUGIN_SLUG, SPT_PLUGIN_ROOT . 'src/Frontend/css/public.min.css', array(), SPT_PLUGIN_VERSION, 'all' );
		}
	...
	public function enqueue_scripts() {
		...
		if ( 'sptp_generator' === $current_post_type ) {
			wp_enqueue_script( 'team-free-swiper', SPT_PLUGIN_ROOT . 'src/Frontend/js/swiper.min.js', array(), SPT_PLUGIN_VERSION, true );
		}
	}
	...
	public function sptp_member_post_type() {
		...
		$labels     = apply_filters(
			...
			array(
				...
				'add_new'               => wp_sprintf( esc_html__( 'Add New', 'team-free' ), $this->sptp_member_singular_name ),
				/* translators: %s is replaced with 'member singular name' */
	...
	public function sptp_generator_post_type() {
		...
		$labels     = apply_filters(
			...
			array(
				...
				'all_items'          => wp_sprintf( __( '%s Generator', 'team-free' ), $this->sptp_team_name ),
	...
	public function sptp_review_text( $text ) {
		...
		if ( ( 'sptp_member' === get_post_type() ) || ( 'sptp_member_page_team_help' === $screen->id ) || ( 'sptp_member_page_team_settings' === $screen->id ) || ( 'sptp_generator' === get_post_type() ) || ( 'edit-sptp_group' === $screen->id ) ) {
			...
			$text = sprintf( wp_kses_post( 'If you like <strong>WP Team</strong>, please leave us a <a href="%s" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a> rating. Your Review is very important to us as it helps us to grow more. ', 'team-free' ), esc_url( $url ) );
		}
