		...
		public static function add_typography_enqueue_styles() {
			...
			if ( ! empty( self::$webfonts ) ) {
				...
				if ( ! empty( self::$webfonts['enqueue'] ) ) {
					...
					wp_enqueue_style( 'spf-google-web-fonts', esc_url( add_query_arg( $query, '//fonts.googleapis.com/css' ) ), array(), '1.0.0', null );
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field_type ) ) {
				...
				if ( ! empty( $field['title'] ) ) {
					...
					$title_help = ( ! empty( $field['title_help'] ) ) ? '<div class="spf-help"><span class="spf-help-text">' . $field['title_help'] . '</span><i class="fas fa-question-circle"></i></div>' : '';

					echo '<h4>' . wp_kses_post( $field['title'] ) . $title_help . '</h4>';
