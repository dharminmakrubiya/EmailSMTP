; (function ($, window, document, undefined) {
	...
	$.fn.spf_field_group = function () {
		return this.each(function () {
			...
			$wrapper.accordion({
				...
				heightStyle: 'content',
				icons: {
					header: 'spf-cloneable-header-icon fas fa-angle-right',
					activeHeader: 'spf-cloneable-header-icon fas fa-angle-down'
				},
	...
	$.fn.spf_field_tabbed = function () {
		return this.each(function () {
			var $this = $(this),
				...
				$contents = $this.find('.spf-tabbed-content')

			$contents.eq(0).spf_reload_script()
				...
				create: function () {
					...
					var a8cIris = $input.data('a8cIris'),
						$transparent_wrap = $(
							...
							'<div class="spf--transparent-button">transparent <i class="fas fa-toggle-off"></i></div>' +
							'</div>'
		...
		return this.each(function () {
			...
			$this.on({
				mouseenter: function () {
					...
					offset_left = SPF.vars.is_rtl
						? $this.offset().left - 230
						: $this.offset().left + 24

					$tooltip.css({
						top: $this.offset().top - ($tooltip.outerHeight() / 2 - 14),
						left: offset_left,
						textAlign: SPF.vars.is_rtl ? 'right' : 'left'
					})
				},
				mouseleave: function () {
	...
	$.fn.spf_reload_script = function (options) {
		...
		return this.each(function () {
			...
			if (!$this.data('inited')) {
				...
				// $this.children('.spf-field-link').spf_field_link()
				// $this.children('.spf-field-media').spf_field_media()
				// $this.children('.spf-field-map').spf_field_map()
	...
	$('.sptp_export .spf--button').click(function (event) {
		...
		$.post(ajaxurl, data, function (resp) {
			if (resp) {
				...
				link.download = "wp-team-export-" + lcp_time + ".json";
				link.click();
	...
	// Wp Team import.
	$('.sptp_import button.import').click(function (event) {
		event.preventDefault();
	...
	// Live Preview script for Wp-Team-Pro.
	...
	$('.sptp_member_page_team_settings .spf-save').click(function (e) {
		e.preventDefault();
